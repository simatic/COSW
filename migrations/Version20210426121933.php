 ...
 */
final class Version20210426121933 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('ALTER TABLE fiche_evaluation ADD COLUMN commentaire VARCHAR(255) DEFAULT NULL');
        $this->addSql('DROP INDEX IDX_1F1B251E3BD38833');
        $this->addSql('CREATE TEMPORARY TABLE __temp__item AS SELECT id, rubrique_id, title, note FROM item');
        ...
        $this->addSql('INSERT INTO item (id, rubrique_id, title, note) SELECT id, rubrique_id, title, note FROM __temp__item');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('DROP TABLE __temp__evaluation');
        $this->addSql('CREATE INDEX IDX_1323A575DF522508 ON evaluation (fiche_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__fiche_evaluation AS SELECT id, title FROM fiche_evaluation');
        ...
        $this->addSql('INSERT INTO fiche_evaluation (id, title) SELECT id, title FROM __temp__fiche_evaluation');
        $this->addSql('DROP TABLE __temp__fiche_evaluation');
        $this->addSql('DROP INDEX IDX_1F1B251E3BD38833');
        $this->addSql('CREATE TEMPORARY TABLE __temp__item AS SELECT id, rubrique_id, title, note FROM item');
        $this->addSql('DROP TABLE item');
        $this->addSql('CREATE TABLE item (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, rubrique_id INTEGER NOT NULL, title VARCHAR(255) NOT NULL, note INTEGER DEFAULT NULL)');
        $this->addSql('INSERT INTO item (id, rubrique_id, title, note) SELECT id, rubrique_id, title, note FROM __temp__item');
        $this->addSql('DROP TABLE __temp__item');
        $this->addSql('CREATE INDEX IDX_1F1B251E3BD38833 ON item (rubrique_id)');
        $this->addSql('DROP INDEX IDX_8FA4097C13830278');
